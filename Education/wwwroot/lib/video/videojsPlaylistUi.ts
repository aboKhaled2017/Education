...
interface UiElement extends JQuery<HTMLElement>{
 thumbnail:Element;
}
    function _inheritsLoose(subClass:(...params:any[])=>any, superClass:new(...params:any[])=>any) {
      subClass.prototype = Object.create(superClass.prototype);
      subClass.prototype.constructor = subClass;
      (subClass as any)['__proto__ ']= superClass;
    }
    var version = "3.5.2";
  
    var dom:videojs.Dom= videojs.dom || (videojs as unknown as videojs.Dom);
    var registerPlugin = videojs.registerPlugin || videojs.plugin; // Array#indexOf analog for IE8
  
    var indexOf = function indexOf(array:Array<any>, target:any) {
    ...
    var addSelectedClass = function addSelectedClass(el:UiElement) {
      el.addClass('vjs-selected');
    };
  
    var removeSelectedClass = function removeSelectedClass(el:UiElement) {
    ...
    var upNext = function upNext(el:UiElement) {
      el.addClass('vjs-up-next');
    };
  
    var notUpNext = function notUpNext(el:UiElement) {
      el.removeClass('vjs-up-next');
    };
  
    var createThumbnail = function createThumbnail(thumbnail:Element) {
      if (!thumbnail) {
    ...
    function (_Component) {
      ...
      function PlaylistMenuItem(player:videojs.Player,playlistItem:{item:any}, settings:{[prop:string]:any}) {
        var _this;
      ...
      _proto.switchPlaylistItem_ = function switchPlaylistItem_(event) {
        if(!PlaylistMenuItem.itemHeight)PlaylistMenuItem.itemHeight=$(`.vjs-playlist-item-list>li:eq(0)`).outerHeight();
       ...
       /* PlaylistMenuItem.maxIndex=this.player_.playlist().length-1;
        ...
        $((this.player_ as videojs.Player).el()).find('#subPlayerSection>p#descEl').text(this.item.description);
