...
var version = "3.5.2";
var dom = video_js_1.default.dom || video_js_1.default;
var registerPlugin = video_js_1.default.registerPlugin || video_js_1.default.plugin; // Array#indexOf analog for IE8
...
function (_Component) {
    ...
    PlaylistMenuItem.tabeIndex = -1;
    PlaylistMenuItem.itemHeight = 0;
    PlaylistMenuItem.maxIndex = 0;
    ...
    _proto.switchPlaylistItem_ = function switchPlaylistItem_(event) {
        if (!PlaylistMenuItem.itemHeight)
            PlaylistMenuItem.itemHeight = $(".vjs-playlist-item-list>li:eq(0)").outerHeight();
        var itemIndex = indexOf(this.player_.playlist(), this.item);
        /* PlaylistMenuItem.maxIndex=this.player_.playlist().length-1;
         var translatedDistance=-(PlaylistMenuItem.itemHeight+5.3)*itemIndex;
         if(PlaylistMenuItem.maxIndex-itemIndex>=2)
         setTimeout(() => {
           $(`.vjs-playlist-item-list`).css({
             transform:`translateY(${translatedDistance}px)`
           });
         },700);*/
        this.player_.playlist.currentItem(itemIndex);
        //switch description el
        $(this.player_.el()).find('#subPlayerSection>p#descEl').text(this.item.description);
        if (this.playOnSelect) {
            this.player_.play();
        }
    };
    _proto.createEl = function createEl() {
        PlaylistMenuItem.tabeIndex++;
        ...
        li.setAttribute('tabIndex', PlaylistMenuItem.tabeIndex); // Thumbnail image
        this.thumbnail = createThumbnail(item.thumbnail);
        li.appendChild(this.thumbnail); // Duration
        if (item.duration) {
            var duration = document.createElement('time');
            var time = video_js_1.default.formatTime(item.duration);
        ...
        var nowPlayingText = this.localize('يعمل الان');
        ...
        var upNextText = this.localize('التالى');
        upNextEl.className = 'vjs-up-next-text hidden';
        upNextEl.appendChild(document.createTextNode(upNextText));
        upNextEl.setAttribute('title', upNextText);
        titleContainerEl.appendChild(upNextEl); // Video title
        var titleEl = document.createElement('cite');
        var titleText = item.name || this.localize('فديو بدون عنوان');
...
function (_Component2) {
    ...
    function PlaylistMenu(player, options) {
        ...
        if (!video_js_1.default.browser.TOUCH_ENABLED) {
...
var playlistUi = function playlistUi(options) {
    ...
    if (dom.isEl(options)) {
        video_js_1.default.log.warn('videojs-playlist-ui: Passing an element directly to playlistUi() is deprecated, use the "el" option instead!');
        options = {
            el: options
        };
    }
    options = video_js_1.default.mergeOptions(defaults, options); // If the player is already using this plugin, remove the pre-existing
...
video_js_1.default.registerComponent('PlaylistMenu', PlaylistMenu);
video_js_1.default.registerComponent('PlaylistMenuItem', PlaylistMenuItem); // register the plugin
registerPlugin('playlistUi', playlistUi);
playlistUi.VERSION = version;
//# sourceMappingURL=videojsPlaylistUi.js.map